...
class PDE_FIND:
    ...
    def TikhonovDiff(self,f, dx, lam, d = 1):
        """
        ...
        else: return self.FiniteDiff(g, dx, d-1)
    ...
    def ConvSmoother(self,x, p, sigma):
        """
    ...
    def PolyDiff(self,u, x, deg = 3, diff = 1, width = 5):
    ...
    def PolyDiffPoint(self,u, x, deg = 3, diff = 1, index = None):
    ...
    def build_linear_system(self,u, dt, dx, D = 3, P = 3,time_diff = 'poly',space_diff = 'poly',lam_t = None,lam_x = None, width_x = None,width_t = None, deg_x = 5,deg_t = None,sigma = 2):
        """
        ...
        if time_diff == 'FDconv':
            ...
            for j in range(m):
                Usmooth[:,j] = self.ConvSmoother(u[:,j],width_t,sigma)
            # Now take finite differences
            for i in range(n2):
                ut[i,:] = self.FiniteDiff(Usmooth[i + offset_x,:],dt,1)

        elif time_diff == 'poly':
            T= np.linspace(0,(m-1)*dt,m)
            for i in range(n2):
                ut[i,:] = self.PolyDiff(u[i+offset_x,:],T,diff=1,width=width_t,deg=deg_t)[:,0]

        elif time_diff == 'Tik':
            for i in range(n2):
                ut[i,:] = self.TikhonovDiff(u[i + offset_x,:], dt, lam_t)

        else:
            for i in range(n2):
                ut[i,:] = self.FiniteDiff(u[i + offset_x,:],dt,1)
        ...
        if space_diff == 'poly': 
            Du = {}
            for i in range(m2):
                Du[i] = self.PolyDiff(u[:,i+offset_t],np.linspace(0,(n-1)*dx,n),diff=D,width=width_x,deg=deg_x)
        ...
        for d in range(D+1):
            ...
            if d > 0:
                for i in range(m2):
                    if space_diff == 'Tik': ux[:,i] = self.TikhonovDiff(u[:,i+offset_t], dx, lam_x, d=d)
                    elif space_diff == 'FDconv':
                        Usmooth = self.ConvSmoother(u[:,i+offset_t],width_x,sigma)
                        ux[:,i] = self.FiniteDiff(Usmooth,dx,d)
                    elif space_diff == 'FD': ux[:,i] = self.FiniteDiff(u[:,i+offset_t],dx,d)
    ...
    def TrainSTRidge(self,R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
        """
    ...
    def Lasso(self,X0, Y, lam, w = np.array([0]), maxit = 100, normalize = 2):
        """
    ...
    def ElasticNet(self,X0, Y, lam1, lam2, w = np.array([0]), maxit = 100, normalize = 2):
        """
    ...
    def FoBaGreedy(self,X, y, epsilon = 0.1, maxit_f = 100, maxit_b = 5, backwards_freq = 5, relearn_f=True, relearn_b=True):
        """
    ...
    def pde_find_algo(self,data):
        ...
        Ut, R, rhs_des = self.build_linear_system(u, dt, dx, D=3, P=3, time_diff = 'FD', space_diff = 'FD')

        # Solve with STRidge
        w = self.TrainSTRidge(R,Ut,10**-5,1)
        print("PDE derived using STRidge")
        print("Method 3 completed")
        
        return self.print_pde(w, rhs_des,)
